    ...
    Seq.iter
      (fun row ->
        let tbl_row =
          ...
          let tbl_type =
            Table.get_table_type (get_string row "TABLE_TYPE" "")
          in
          let tbl_engine = Table.get_engine (get_string row "ENGINE" "") in
          (tbl_name, tbl_type, tbl_engine)
        in
        rows := tbl_row :: !rows)
      (M.stream res);
    List.rev !rows
...
let create_index_map conn db_name =
  ...
  Seq.iter
    (fun row ->
      ...
      let idx_type =
        Core_types.Index.parse_index_type (get_string row "INDEX_TYPE" "BTREE")
      in
