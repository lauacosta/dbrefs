...
let or_die where = function
  | Ok r -> r
  | Error e -> failwith (Printf.sprintf "%s: %s" where (E.show e))
...
let parse_scheme s =
  ...
  | None ->
      Error (E.Dsn_scheme_error "Missing scheme (expected something://...)")
  | Some (scheme, rest) ->
      if String.length rest >= 2 && rest.[0] = '/' && rest.[1] = '/' then
        Ok (scheme, String.sub rest 2 (String.length rest - 2))
      else
        Error (E.Dsn_scheme_error "Malformed scheme: expected // after scheme")
...
module Dsn = struct
  ...
  let driver_of_string = function
    ...
    | other -> Error (E.Unsupported_driver ("Unknown driver: " ^ other))

  let parse_dsn input =
    match parse_scheme input with
    | Error e -> Error (E.Invalid_DSN (input ^ ": " ^ E.show e))
    | Ok (scheme, rest) -> (
