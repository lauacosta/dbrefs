...
(* Don't use this function eagerly because the internal buffer in Mariadb will refer to the last seen data *)
let stream res =
  let module F = struct
...
let query_map conn db_name ~query ~row_to_kv ~merge =
  let stmt = M.prepare conn query |> or_die "prepare query" in
  let res = M.Stmt.execute stmt [| `String db_name |] |> or_die "exec query" in
...
let create_table_rows conn db_name =
  ...
  in
  let stmt = M.prepare conn tables_query |> or_die "prepare tables" in
  let res = M.Stmt.execute stmt [| `String db_name |] |> or_die "exec tables" in
...
let create_index_map conn db_name =
  ...
  in
  let stmt = M.prepare conn index_query |> or_die "prepare index" in
  let res = M.Stmt.execute stmt [| `String db_name |] |> or_die "exec index" in
